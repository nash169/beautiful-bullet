...
namespace robot_bullet {
    namespace utils {
        ...
        void ComputeTotalNumberOfJoints(const importers::ImporterURDF& u2b, URDF2BulletCachedData& cache, int linkIndex)
        {
        ...
        void ComputeParentIndices(const importers::ImporterURDF& u2b, URDF2BulletCachedData& cache, int urdfLinkIndex, int urdfParentIndex)
        {
        ...
        void InitURDF2BulletCache(const importers::ImporterURDF& u2b, URDF2BulletCachedData& cache, int flags)
        {
        ...
        btTransform ConvertURDF2BulletInternal(
            const importers::ImporterURDF& u2b, MultiBodyCreator& creation,
            ...
            int flags, UrdfVisualShapeCache* cachedLinkGraphicsShapesIn, UrdfVisualShapeCache* cachedLinkGraphicsShapesOut, bool recursive)
        {
            ...
            if (urdfParentIndex == -2) {
                ...
                u2b.getMassAndInertia(urdfParentIndex, parentMass, parentLocalInertiaDiagonal, parentLocalInertialFrame, flags);
            }
            ...
            int graphicsIndex;
            {
                ...
                if (cachedLinkGraphicsShapesIn && cachedLinkGraphicsShapesIn->m_cachedUrdfLinkVisualShapeIndices.size() > (mbLinkIndex + 1)) {
                    ...
                    // Comment for now check if it is useful
                    ...
                    case URDFRevoluteJoint: {
                        ...
                        if (createMultiBody) {
                            cache.m_bulletMultiBody->setupRevolute(mbLinkIndex, mass, localInertiaDiagonal, mbParentIndex,
                                parentRotToThis, quatRotate(offsetInB.getRotation(), jointAxisInJointSpace),
                                offsetInA.getOrigin(),
                                -offsetInB.getOrigin(),
                                disableParentCollision);
        ...
        void GetAllIndices(const importers::ImporterURDF& u2b, URDF2BulletCachedData& cache, int urdfLinkIndex, int parentIndex, btAlignedObjectArray<childParentIndex>& allIndices)
        {
            childParentIndex cp;
