#include "robot_bullet/utils/ResourcePath.hpp"

namespace robot_bullet {
    namespace utils {
        int ResourcePath::getExePath(char* path, int maxPathLenInBytes)
        {
            int numBytes = 0;
        ...
        void ResourcePath::setAdditionalSearchPath(const char* path)
        {
            if (path) {
                int len = strlen(path);
        ...
        int ResourcePath::findResourcePath(const char* resourceName, char* resourcePathOut, int resourcePathMaxNumBytes, PFN_FIND_FILE findFile, void* userPointer)
        {
            if (findFile == 0) {
            ...
            int l = ResourcePath::getExePath(exePath, B3_MAX_EXE_PATH_LEN);
            if (l) {
