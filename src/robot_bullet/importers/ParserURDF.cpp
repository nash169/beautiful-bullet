...
namespace robot_bullet {
    namespace importers {
        ...
        ParserURDF::ParserURDF(interfaces::FileIOInterface* fileIO)
            : m_parseSDF(false),
              m_activeSdfModel(-1),
              m_urdfScaling(1),
              m_fileIO(fileIO)
 ...
 expected to free the memory, for example with str_array_free. */
        ...
        // </bullet>
        static void ParseUserData(const XMLElement* element, btHashMap<btHashString, std::string>& user_data, ErrorLogger* logger)
        {
        ...
        bool ParserURDF::parseMaterial(UrdfMaterial& material, XMLElement* config, ErrorLogger* logger)
        {
        ...
        bool ParserURDF::parseTransform(btTransform& tr, XMLElement* xml, ErrorLogger* logger, bool parseSDF)
        {
            tr.setIdentity();
        ...
        bool ParserURDF::parseInertia(UrdfInertia& inertia, XMLElement* config, ErrorLogger* logger)
        {
            ...
            if (m_parseSDF) {
                ...
                if (!(inertia_xml->Attribute("ixx") && inertia_xml->Attribute("ixy") && inertia_xml->Attribute("ixz") && inertia_xml->Attribute("iyy") && inertia_xml->Attribute("iyz") && inertia_xml->Attribute("izz"))) {
                    if ((inertia_xml->Attribute("ixx") && inertia_xml->Attribute("iyy") && inertia_xml->Attribute("izz"))) {
        ...
        bool ParserURDF::parseGeometry(UrdfGeometry& geom, XMLElement* g, ErrorLogger* logger)
        {
            //	btAssert(g);
            if (g == 0)
                return false;
        ...
        bool ParserURDF::parseCollision(UrdfCollision& collision, XMLElement* config, ErrorLogger* logger)
        {
        ...
        bool ParserURDF::parseVisual(UrdfModel& model, UrdfVisual& visual, XMLElement* config, ErrorLogger* logger)
        {
        ...
        bool ParserURDF::parseLink(UrdfModel& model, UrdfLink& link, XMLElement* config, ErrorLogger* logger)
        {
...
//modified version of SDF audio_source specification in //http://sdformat.org/spec?ver=1.6&elem=link
        ...
        bool ParserURDF::parseLameCoefficients(LameCoefficients& lameCoefficients, tinyxml2::XMLElement* config, ErrorLogger* logger)
        {
        ...
        bool ParserURDF::parseDeformable(UrdfModel& model, tinyxml2::XMLElement* config, ErrorLogger* logger)
        {
        ...
        bool ParserURDF::parseJointLimits(UrdfJoint& joint, XMLElement* config, ErrorLogger* logger)
        {
        ...
        bool ParserURDF::parseJointDynamics(UrdfJoint& joint, XMLElement* config, ErrorLogger* logger)
        {
        ...
        bool ParserURDF::parseJoint(UrdfJoint& joint, XMLElement* config, ErrorLogger* logger)
        {
            // Get Joint Name
        ...
        bool ParserURDF::parseSensor(UrdfModel& model, UrdfLink& link, UrdfJoint& joint, XMLElement* config, ErrorLogger* logger)
        {
        ...
        bool ParserURDF::mergeFixedLinks(UrdfModel& model, UrdfLink* link, ErrorLogger* logger, bool forceFixedBase, int level)
        {
            ...
            for (int i = 0; i < link->m_childJoints.size();) {
                if (link->m_childJoints[i]->m_type == URDFFixedJoint) {
                    ...
                    for (int g = 0; g < childLink->m_childJoints.size(); g++) {
                        ...
                        grandChildJoint->m_parentLinkToJointTransform = childJoint->m_parentLinkToJointTransform * grandChildJoint->m_parentLinkToJointTransform;
        ...
        bool ParserURDF::printTree(UrdfLink* link, ErrorLogger* logger, int level)
        {
            printf("\n");
            for (int l = 0; l < level; l++) {
                printf("\t");
        ...
        bool ParserURDF::recreateModel(UrdfModel& model, UrdfLink* link, ErrorLogger* logger)
        {
            if (!link->m_parentJoint) {
        ...
        bool ParserURDF::initTreeAndRoot(UrdfModel& model, ErrorLogger* logger)
        {
            ...
            if (model.m_rootLinks.size() > 1) {
                std::string multipleRootMessage = "URDF file with multiple root links found:";
        ...
        bool ParserURDF::loadUrdf(const char* urdfText, ErrorLogger* logger, bool forceFixedBase, bool parseSensors)
        {
            XMLDocument xml_doc;
        ...
        void ParserURDF::activateModel(int modelIndex)
        {
            m_activeSdfModel = modelIndex;
        }

        bool ParserURDF::loadSDF(const char* sdfText, ErrorLogger* logger)
        {
            XMLDocument xml_doc;
        ...
        std::string ParserURDF::sourceFileLocation(XMLElement* e)
        {
#if 0
